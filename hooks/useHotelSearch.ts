...
import type { AsyncState, CurrencyCode, HotelCandidate, HotelGrade, HotelPreference, LodgingType, SearchSource } from "@/types";
...
export function useHotelSearch() {
  ...
  const run = useCallback(async (params: SearchParams) => {
    ...
    try {
      const data = await postJson<SearchResult>("/api/find-hotels", params, controller.signal);
      setResult(data);
