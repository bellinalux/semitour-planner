...
export function buildFlightOptionsResearchPrompt(req: FlightOptionsRequest): string {
  ...
  return [
    ...
    "",
    "항공편마다 아래를 조사 메모로 정리하세요. 확인하지 못한 항목은 '확인 못함'이라고 쓰세요.",
    "1. 항공사, 편명(가는 편 기준)",
    "2. 출발일, 출발 공항(코드 포함), 출발 시각",
    "3. 도착 공항(코드 포함), 도착 시각",
    "4. 경유 횟수(직항이면 직항이라고 명시), 총 비행시간(경유 대기 포함)",
    "5. 왕복 1인 요금(이코노미, 세금·유류할증료 포함 여부도 적기)",
    "6. 요금을 확인한 사이트 이름과, 그 항공편을 다시 찾아볼 수 있는 URL",
  ].join("\n");
}
...
- stops는 직항이면 0, 경유 1회면 1입니다. 메모에 명시가 없으면 0으로 두지 말고 실제 조사된 값을 씁니다(불확실하면 그 항공편은 결과에서 뺍니다).
