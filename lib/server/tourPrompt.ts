...
export function buildTourResearchPrompt(req: TourRequest): string {
  ...
  return [
    ...scopeLines,
    "",
    operator ? "찾을 투어 종류 (해당하는 것만, 무리해서 종류를 다 채우지 않아도 됩니다):" : "찾을 투어 종류 (종류마다 1~3개, 전체 6~10개):",
    ...kinds,
    "",
    ...
    "3. 소요 시간",
    `4. 1인 요금 범위 (${req.currency} 기준, 어느 사이트에서 확인했는지)`,
    "5. 요금에 포함되는 것 (입장권, 가이드, 식사, 이동수단 등)",
    "6. 예약 필요 여부, 집합 장소, 운영 요일",
    "7. 한국어 가이드나 한국어 후기가 있는지 (있다면 근거와 출처 사이트)",
    "8. 추천 이유 한 줄",
    ...(operator ? [] : ["", "가격대와 성격이 서로 다른 투어를 골고루 넣어 주세요."]),
  ].join("\n");
}
